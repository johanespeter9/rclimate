...
TZA <- st_read("data/Dodoma.gpkg") %>% st_make_valid()
...
ggplot() +
  ...
  labs(title = "MONTHLY MEAN TEMPERATURE FROM January 2023-December 2023", fill = "Temperature (°C)") +
  theme_minimal()

# Save the plot as PDF
ggsave("data/dodoma2.pdf", width = 16.5, height = 11.7)

# Save the plot as JPG
ggsave("data/dodoma2.jpg", width = 19.8, height = 10.2, units = "in", dpi = 300)
